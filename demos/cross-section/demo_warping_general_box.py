...
Iwx = assemble_scalar(form(((x[0]-x_G)*uh)*dx))
Iwy = assemble_scalar(form(((x[1]-y_G)*uh)*dx))
...
plotter.add_mesh(u_grid.warp_by_scalar("w",factor=1), show_edges=True)
plotter.view_vector((-0.25,-1,0.5))
if not pyvista.OFF_SCREEN:
    plotter.show()
...
glyphs = function_grid.glyph(orient="grad_u", factor=0.1)
