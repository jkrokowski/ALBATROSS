...
def get_elasticity_tensor(E,nu):
...
def decouple_modes(sols,V):
    ...
    return sols_decoup,ubar_vtx_to_dof,uhat_vtx_to_dof

def compute_flex(domain,Ve,sols_decoup,ubar_vtx_to_dof,uhat_vtx_to_dof):
    #==================================================#
...
'''
FXN: construct Mesh geometry in gmsh
     -if homogenous, can just provide E,nu,rho and get elasticity tensor from
     -if heterogenous, need to label subregion in mesh with meshtags that correspond to material as data structure:
          -[mat region,matID]
FXN: get_elasticity_tensor_iso(E,nu)
     get_elasticity_tensor_ortho(?)
     get_elasticity_tensor_aniso(?)

     -returns C_mat, a dolfin tensor

DATA STRUCTURE: mat_array = [[matIDs],[C],[rho]]
*NEED TO GENERALIZE TO ANISOTROPIC

FXN: C,rho = construct_mat_fxn(domain,mat_array)
          -C: tensor fxn over domain
          -rho: scalar fxn over domain

FXN: K,M = compute_xc_prop(domain,C,rho)
     -Enter material parameters, mesh
     -return returns 
assemble matrix
get modes
decouple modes (elastic from rigid)
compute flexibility matrix
'''
...
C = get_elasticity_tensor(E,nu)

def get_xc_prop(domain,C):
    V,Ve = get_fxn_space(domain)
    residual = get_residual(domain,V,C)
    sols = get_modes(residual)
    sols_decoup,ubar_vtx_to_dof,uhat_vtx_to_dof = decouple_modes(sols,V)
    compute_flex(domain,Ve,sols_decoup,ubar_vtx_to_dof,uhat_vtx_to_dof)
