...
boundary_points = csdl.concatenate([xy[list(xs.boundary_ordering)],xy[list(xs.boundary_ordering)][:1,:]])
# parametric_coords = np.array([(i,) for i in np.linspace(0,1,xs.boundary_nodes.shape[0]+bspline_degree)])
# boundary_points = csdl.concatenate([xy[list(xs.boundary_ordering)],xy[list(xs.boundary_ordering)][:bspline_degree,:]]) #duplicate the first 3 start points as the endpoints
...
inputs.coeffs.set_as_design_variable(lower=-.25,upper=.25,scaler=500)
...
xy = spline_eval.get(csdl.slice[list(xs.inverse_boundary_ordering)])
xy.name = 'xy'
...
with csdl.namespace('Maximum x coord'):
    g3 = csdl.maximum(csdl.absolute(xy[:,0])) 
    g3.add_name('g3')
    g3.set_as_constraint(upper=.05)

with csdl.namespace('Maximum y coord'):
    g4 = csdl.maximum(csdl.absolute(xy[:,1])) 
    g4.add_name('g4')
    g4.set_as_constraint(upper=.05)
...
# dddx = sim.check_totals(tip_displacement,inputs.coeffs,step_size=0.00001)
