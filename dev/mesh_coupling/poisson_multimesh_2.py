...
M1T= M1.copy().transpose()
M2T= M2.copy().transpose()

# A1M2T = A1.matMult(M2T)
# A2M1T = A2.matMult(M1T)
...
# M1A1M1T = M1.matMult(A1M1T)
# M2A2M2T = M2.matMult(A2M2T)

# M1A1M1T = M1A1.matMult(M1T)
# A2.axpy(1.0,M1A1M1T)
# M2A2M2T = M2A2.matMult(M2T)
# A1.axpy(1.0,M2A2M2T)
...
#               [-M1A1,A2]])
A.createNest([[A1,A1M1T],
              [A2M1,A2]])
# A.createNest([[A1,A1M2],
#               [A2M1,A2]])
# A.createNest([[A1,A1M2T],
#               [A2M1T,A2]])
# A.createNest([[A1,A1M1],
#               [A2M2,A2]])
# A.createNest([[A1,A1M1],
            #   [A2M1T,A2]])
# A.createNest([[A1,A1M2T],
...
uh2_max = np.max(uh2.x.array)

plotter = pyvista.Plotter()
plotter.add_text("uh", position="upper_edge", font_size=14, color="black")
plotter.add_mesh(grid1, show_edges=True,opacity=.5,clim=[0,np.max([uh1_max,uh2_max])])
plotter.add_mesh(grid2, show_edges=True,opacity=.5,clim=[0,np.max([uh1_max,uh2_max])])
